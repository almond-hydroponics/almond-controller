...
std::basic_string<char> deviceId = std::to_string(AlmondConfiguration::get_device_id());
String device_topic =
		reinterpret_cast<String&&>(deviceId.append("almond/data"));
...
MqttServer mqttClient(mqttServer,
									mqttUsername,
									mqttPassword,
...
void onConnectionEstablished() {
	...
	mqttClient.subscribe(topic_pump, [](const String& payload) {
...
//									 "This is a message sent after 5 seconds later.");
//		LOG_INFO("Published to %08X/test", ESP.getChipId());
//	});
...
void loop() {
	...
	if (update_when_elapsed) {
		if (update_timer.check(UPDATE_DELAY * 1000)) {
			...
			if (valid) {
				LOG_INFO(
						"Values read successfully. Replace with push function to azure or "
						"GCP ####");
			} else {
	...
	if (logic_changed) {
		...
		// if new status is idle, then we were pumping -> push the delays (assuming
		// valid)
